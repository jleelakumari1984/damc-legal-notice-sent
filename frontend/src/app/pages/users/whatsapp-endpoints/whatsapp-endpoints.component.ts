...
import { WhatsappEndpointService } from '../../../core/services/whatsapp-endpoint.service';
...
import { WhatsappEndpoint } from '../../../core/models/endpoint.model';
...
export class WhatsappEndpointsComponent implements OnInit, OnDestroy {
  private readonly TABLE_ID = 'whatsappEndpointsTable';
  private readonly dtHelper = new DatatableHelper();

  editEndpoint: WhatsappEndpoint | null = null;
  form!: FormGroup;
  saving = false;
  errorMessage = '';
  formError = '';

  constructor(private readonly fb: FormBuilder, private readonly service: WhatsappEndpointService) {}
    ...
    this.dtHelper.initWithAjax(this.TABLE_ID, {
      ...
      ajax: (_dtParams: any, callback: (data: object) => void) => {
        this.service.getAll().subscribe({
          ...
          error: () => {
            this.errorMessage = 'Failed to load WhatsApp endpoints.';
            callback({ data: [] });
          }
        });
      },
      columns,
      createdRow: (row: HTMLElement, data: WhatsappEndpoint) => {
  ...
  openForm(endpoint?: WhatsappEndpoint): void {
    ...
    if (endpoint) {
      ...
      this.form.reset({ port: 443, status: 1 });
    }
    $('#whatsappEndpointModal').modal('show');
  }

  cancel(): void {
    $('#whatsappEndpointModal').modal('hide');
  }
  ...
  submit(): void {
    ...
    call$.subscribe({
      ...
      error: () => {
        this.saving = false;
        this.formError = 'Failed to save WhatsApp endpoint.';
      }
    });
  }

  delete(endpoint: WhatsappEndpoint): void {
    if (!confirm(`Delete WhatsApp endpoint "${endpoint.name}"?`)) return;
