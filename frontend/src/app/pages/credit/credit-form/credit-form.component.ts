...
export class CreditFormComponent implements OnInit, OnChanges, OnDestroy {
  @Input() editCredit: Credit | null = null;
  ...
  submit(): void {
    ...
    const call = this.editCredit
      ? this.service.update(this.editCredit.id, request)
    ...
    call.subscribe({
      ...
      error: () => {
        this.errorMessage = this.editCredit ? 'Failed to update record.' : 'Failed to create record.';
        this.saving = false;
      }
    });
  }
