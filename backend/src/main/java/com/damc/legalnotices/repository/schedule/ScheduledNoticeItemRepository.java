...
public interface ScheduledNoticeItemRepository extends JpaRepository<ScheduledNoticeItemEntity, Long> {
    ...
    List<ScheduledNoticeItemEntity> findByScheduledNoticeIdAndStatus(Long scheduledNoticeId, NoticeScheduleStatus status);
    ...
    @Modifying
    @Query(value = "UPDATE scheduled_notice_items SET identifier = :identifier, status = :NoticeScheduleStatus WHERE status = :pendingStatus ORDER BY created_at ASC LIMIT 50", nativeQuery = true)
    int claimPendingItems(long identifier, String NoticeScheduleStatus, String pendingStatus);
}
