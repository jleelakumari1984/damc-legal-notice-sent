...
public interface ScheduledNoticeItemRepository extends JpaRepository<ScheduledNoticeItemEntity, Long> {
        ...
        @Modifying
        @Query(value = "UPDATE ScheduledNoticeItemEntity SET status = :updateStatus WHERE scheduledNotice.id = :noticeId and status = 'PENDING'")
        void updateStopStatus(@Param("noticeId") Long noticeId, @Param("updateStatus") String updateStatus);

        @Modifying
        @Query(value = "UPDATE ScheduledNoticeItemEntity SET status = :updateStatus WHERE scheduledNotice.id = :noticeId AND (status = 'STOP' or status = 'UPLOADED')")
        void updateStartStatus(@Param("noticeId") Long noticeId, @Param("updateStatus") String updateStatus);
}
